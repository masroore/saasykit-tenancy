...
COPY --link --chown=${WWWUSER}:${WWWUSER} deployment/octane/Franken/supervisord.frankenphp.conf /etc/supervisor/conf.d/
...
FROM node:20-alpine as build-npm
...
#COPY --link --from=common ./resources ./src
#COPY --link --from=common ./public ./public
...
COPY --link --chown=${WWWUSER}:${WWWUSER} --from=build-npm ${ROOT}/public public
